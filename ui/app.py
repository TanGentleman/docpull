...
# Import config (generated by setup.py)
try:
    from config.settings import SCRAPER_API_URL, IS_PROD
except ImportError:
    # Fallback for when running in Modal (settings.py is in /root/)
    import sys
    sys.path.insert(0, "/root")
    try:
        from settings import SCRAPER_API_URL, IS_PROD
    except ImportError:
        raise ImportError(
            "\n" + "=" * 70 + "\n"
            "ERROR: UI configuration not found!\n\n"
            "Before running the UI, you need to configure the API URL:\n\n"
            "  1. Run 'python setup.py' to deploy and auto-configure, or\n"
            "  2. Set the SCRAPER_API_URL environment variable\n"
            + "=" * 70
        )
...
image = (
    ...
    .add_local_file("config/settings.py", "/root/settings.py")
)
...
def get_scraper_api_url():
    """Get scraper API URL from environment or config."""
    # Environment variable takes precedence (useful for production)
    env_url = os.environ.get("SCRAPER_API_URL")
    if env_url:
        return env_url
