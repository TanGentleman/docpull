...
ACCESS_KEY=your-secret-key  # Optional, if deployed with --access-key
```

## Authentication

If deployed with `--access-key`, all API requests (except `/health`) require the `X-Access-Key` header:

```bash
curl -H "X-Access-Key: your-secret-key" https://your-api/sites
```

The CLI automatically includes this header when `ACCESS_KEY` is set in `.env`.
...
# Deploy
python deploy.py --app-name myapp --access-key secret123
```
